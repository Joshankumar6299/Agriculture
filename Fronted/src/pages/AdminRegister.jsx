        ...
        <form onSubmit={handleRegister} className="space-y-5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select value={gender} onChange={(e) => setGender(e.target.value)} disabled={loading} required className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-600">
          ...
          <div>
            ...
            <textarea value={address} onChange={(e) => setAddress(e.target.value)} disabled={loading} required className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-600" rows="3"></textarea>
          </div>
          <Input label="Profile Image" type="file" onChange={(e) => setProfileImage(e.target.files[0])} disabled={loading} />
          <button type="submit" className="w-full bg-indigo-700 text-white font-bold py-3 rounded-lg hover:bg-indigo-800 transition-colors disabled:bg-indigo-400" disabled={loading}>
...
const Input = ({ label, type, value, onChange, disabled, required }) => (
  <div>
    ...
    <input type={type} value={value} onChange={onChange} disabled={disabled} required={required} className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-indigo-50 file:text-indigo-700 hover:file:bg-indigo-100" />
  </div>
);
